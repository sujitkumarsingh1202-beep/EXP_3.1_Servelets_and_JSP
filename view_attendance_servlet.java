...
public class ViewAttendanceServlet extends HttpServlet {
    ...
    private void displayStatistics(PrintWriter out, Connection conn) {
        ...
        try {
            ...
            // Late count
            rs = stmt.executeQuery("SELECT COUNT(*) as count FROM Attendance WHERE Status = 'Late'");
            if (rs.next()) {
                out.println("<div class='stat-card'>");
                out.println("<div class='stat-value' style='color: #f59e0b;'>" + rs.getInt("count") + "</div>");
                out.println("<div class='stat-label'>Late</div>");
                out.println("</div>");
            }
